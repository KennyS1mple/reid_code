...
# Desc     : get object of interest (with 2 labels)
        ...
        if _img in label_total:
            ...
            with open(os.path.join(label_path, filename_label), "r+", encoding='utf-8', errors="ignor") as f:
                for line in f:
                    ...
                    x1 = int((float(msg[2]) - float(msg[4]) / 2) * w)  # x_center - width/2
                    y1 = int((float(msg[3]) - float(msg[5]) / 2) * h)  # y_center - height/2
                    x2 = int((float(msg[2]) + float(msg[4]) / 2) * w)  # x_center + width/2
                    y2 = int((float(msg[3]) + float(msg[5]) / 2) * h)  # y_center + height/2
                    print(filename_img)
